...
func cheat_shoot(cast):
	for enemy in vision_enemies:
		...
		new_bullet.direction = cast.global_position.direction_to(enemy.global_position)
		...
		second_bullet.direction = cast.global_position.direction_to(enemy.global_position).rotated(deg_to_rad(sin(time * frequency) * spread))
